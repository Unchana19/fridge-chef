      ...
      <CardBody className="p-5">
        ...
        <div className="flex flex-col gap-2 w-full justify-start">
          {items.map((item) => (
            <div
              key={`shop-${item}`}
              className="flex w-full items-center gap-2 hover:bg-warning/10 rounded-lg py-1 px-2 -mx-2 transition-colors cursor-pointer"
            >
              <Checkbox
                color="warning"
                radius="full"
              ...
              />
              <span className="text-warning-800 dark:text-warning-200 font-medium text-sm capitalize">
                {item}
              </span>
            </div>
          ))}
